    ...
    const canvas=document.getElementById('canvas')
     ...
     function moveBall(){
          ...
          //上下
          if(ball.y+ball.size>canvas.height||ball.y-ball.size<0){//圆的原点在圆心
               ball.dy*=-1//相反数，相当于反弹
          }

           //设置挡板反弹边界
          //把挡板用边界包起来，来实现碰撞区域
          if(ball.y+ball.size>plank.y&&//挡板顶部
          ...
          bricks.forEach(column=>{
               column.forEach(brick=>{
                    if(brick.visible){
                         //把砖块用边界包起来，来实现碰撞区域
                         if(ball.y+ball.size>brick.y&&//砖块顶部
                              ball.x-ball.size>brick.x&&//砖块左侧
                              ball.x+ball.size<brick.x+brick.w&&//砖块右侧
                              ball.y-ball.size<brick.y+brick.h){//砖块底侧
                                   ball.dy*=-1
